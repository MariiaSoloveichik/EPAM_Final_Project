...
def weather_data_update(request):
    ...
    requested_date_from = datetime.strptime(
        request["form_start_date"], "%d-%m-%Y"
    ).date()
    requested_date_to = datetime.strptime(request["form_end_date"], "%d-%m-%Y").date()

    if not checking_city_in_database(requested_city):
        download_data_batches(
            {
                "form_city": requested_city,
                "form_start_date": requested_date_from,
                "form_end_date": requested_date_to,
            }
        )

    if (
        requested_date_to
        > City.objects.filter(city_name__startswith=requested_city).last().date
    ):
        # the start date to upload data (last date from db + 1 day)
        request_start_day = City.objects.filter(
            city_name__startswith=requested_city
    ...
    if (
        requested_date_from
        < City.objects.filter(city_name__startswith=requested_city).first().date
    ):
        # the start date to upload data (last date from db + 1 day)
        request_end_day = City.objects.filter(
            city_name__startswith=requested_city
        ).first().date + timedelta(days=1)
        request_upd = {
            "form_city": requested_city,
            "form_start_date": requested_date_from,
            "form_end_date": request_end_day,
        }
        download_data_batches(request_upd)
...
def upload_data_to_base(data):
    ...
    for date in data["data"]["weather"]:
        ...
        day_record.precip_mm = sum([float(time["precipMM"]) for time in hourly]) / len(
            hourly
        )
        ...
        day_record.wind_speed = sum(
            [int(time["windspeedKmph"]) for time in hourly]
        ) / len(hourly)
...
def download_data_batches(request: dict):
    ...
    start_date = request["form_start_date"]
    end_date = request["form_end_date"]
...
def preparing_data_to_show(request) -> dict:
    ...
    if start_date:
        temp_start = datetime.strptime(
            datetime.strptime(start_date, "%d-%m-%Y").strftime("%Y-%m-%d"), "%Y-%m-%d"
        ).date()

    if end_date:
        temp_end = datetime.strptime(
            datetime.strptime(end_date, "%d-%m-%Y").strftime("%Y-%m-%d"), "%Y-%m-%d"
        ).date()
    ...
    data_to_show = City.objects.filter(
        ...
        date__range=(temp_start, temp_end),
    )
    ...
    context["min_temp_per_period"] = data_to_show.aggregate(Min("min_temp"))[
        "min_temp__min"
    ]
    # b) the average temperature
    context["avg_temp_per_period"] = round(
        data_to_show.aggregate(Avg("avg_temp"))["avg_temp__avg"], 1
    )
    # c) the absolute maximum for the period
    context["max_temp_per_period"] = data_to_show.aggregate(Max("max_temp"))[
        "max_temp__max"
    ]

    # d) if the period is more than 2 years:
    # i) average maximum by the years
    context["years_avg_min"] = (
        data_to_show.values("date_year").annotate(Avg("min_temp")).order_by("date_year")
    )
    # ii) average minimum by the years
    context["years_avg_max"] = (
        data_to_show.values("date_year").annotate(Avg("max_temp")).order_by("date_year")
    )
    ...
    days_without_precipitation = (
        data_to_show.annotate(Count("precip_mm")).filter(precip_mm=0).count()
    )
    context["percent_days_of_precipitation"] = 100 - round(
        days_without_precipitation
        / data_to_show.annotate(days=Count("precip_mm")).count()
        * 100
    )
    # b) two most common types of precipitation for the  period
    context["most_common_weather_per_period"] = (
        data_to_show.values("most_common_weather")
        .annotate(count=Count("most_common_weather"))
        .order_by("-count")[:2]
    )
    ...
    context["avg_wind_direction"] = round(find_average_wind_direction(wind_directions))
